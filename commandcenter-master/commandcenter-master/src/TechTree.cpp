...
void TechTree::outputJSON(const std::string & filename) const
{
    ...
    for (auto & kv : allData)
    {
        ...
        { 
            out << q << m_bot.Observation()->GetUnitTypeData()[data.whatBuilds[i]].name << q;
        ...
        { 
            out << q << m_bot.Observation()->GetUnitTypeData()[data.requiredUnits[i]].name << q;
